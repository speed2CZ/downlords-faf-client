...
public class ModVaultController extends AbstractViewController<Node> {
  ...
  private void searchByQuery(String query) {
    ...
    currentPage = 0;
    enterLoadingState();
    displayModsFromSupplier(() -> modService.findByQuery(query, ++currentPage, MAX_SEARCH_RESULTS));
  }
  ...
  private void displayShowroomMods() {
    enterLoadingState();
    modService.getMostPlayedMods(TOP_ELEMENT_COUNT, 1).thenAccept(mods -> replaceSearchResult(mods, mostPlayedPane))
        .thenCompose(aVoid -> modService.getHighestRatedMods(TOP_ELEMENT_COUNT, 1)).thenAccept(mods -> replaceSearchResult(mods, mostLikedPane))
        .thenCompose(aVoid -> modService.getNewestMods(TOP_ELEMENT_COUNT, 1)).thenAccept(mods -> replaceSearchResult(mods, newestPane))
  ...
  private void onShowModDetail(Mod mod) {
  ...
  public void onRefreshButtonClicked() {
    ...
    switch (state) {
      case SHOWROOM:
        ...
        break;
      case SEARCH_RESULT:
        displayModsFromSupplier(() -> modService.findByQuery(query, 1, MAX_SEARCH_RESULTS));
        break;
      default:
        // Do nothing
    }
  ...
  public void showMoreMostRecentMods() {
    enterLoadingState();
    displayModsFromSupplier(() -> modService.getNewestMods(LOAD_MORE_COUNT, ++currentPage));
  }

  public void showMoreMostPlayedMods() {
    enterLoadingState();
    displayModsFromSupplier(() -> modService.getMostPlayedMods(LOAD_MORE_COUNT, ++currentPage));
  }
  ...
  private void appendSearchResult(List<Mod> mods, Pane pane) {
    ...
    List<ModCardController> controllers = mods.parallelStream()
        .map(mod -> {
          ModCardController controller = uiService.loadFxml("theme/vault/map/mod_card.fxml");
